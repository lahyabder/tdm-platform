        ...
        <div className="min-h-screen">
            ...
            <section className="bg-brand-dark py-24 px-6 text-white relative overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto relative z-10 space-y-6">
                    <Link href={`/${locale}/projects`} className="text-xs font-bold text-slate-300 hover:text-white transition-colors flex items-center gap-2 uppercase tracking-widest">
                ...
                <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-16">
                    ...
                    <div className="md:col-span-2 space-y-12">
                        <div className="space-y-6">
                            <h2 className="text-sm font-black text-slate-300 uppercase tracking-[0.2em]">{t.details}</h2>
                        ...
                        <div className="space-y-6">
                            <h2 className="text-sm font-black text-slate-300 uppercase tracking-[0.2em]">{t.goals}</h2>
                            <ul className="grid grid-cols-1 gap-4">
                                {project.goals[locale as 'ar' | 'fr'].map((goal, idx) => (
                                    <li key={idx} className="flex items-start gap-4 p-4 bg-brand-card border border-white/10 rounded-xl group hover:border-brand-green hover:bg-white transition-all">
                                        ...
                                        <span className="text-white font-bold self-center">{goal}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
                    ...
                    <div className="space-y-8">
                        <div className="bg-brand-card rounded-2xl p-8 border border-white/10 space-y-6">
                            ...
                            <div className="space-y-2">
                                <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">{t.startDate}</p>
                                <p className="text-lg font-bold text-white">{project.startDate}</p>
                            </div>
                            {project.completionDate && (
                                <div className="space-y-2">
                                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">{t.compDate}</p>
                                    <p className="text-lg font-bold text-white">{project.completionDate}</p>
                                </div>
                            )}
                        ...
                        <div className="bg-brand-red/5 rounded-2xl p-8 border border-brand-red/10">
                            ...
                            <p className="text-sm font-bold text-slate-200">Contactez le département des projets stratégiques pour toute question technique.</p>
                        </div>
                    </div>
