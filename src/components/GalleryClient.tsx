...
export function GalleryClient({ locale }: { locale: string }) {
    ...
    return (
        <main className="min-h-screen bg-brand-dark">
            ...
            <section className="relative pt-28 pb-24 overflow-hidden">
                ...
                <div className="absolute top-1/4 start-1/4 w-96 h-96 bg-brand-green/10 rounded-full blur-[120px]" />
                <div className="absolute bottom-0 end-1/4 w-72 h-72 bg-brand-yellow/10 rounded-full blur-[100px]" />
            ...
            <div className="sticky top-16 z-30 bg-brand-dark/80 backdrop-blur-xl border-b border-white/5">
                <div className="max-w-6xl mx-auto px-6 py-4 flex gap-3 flex-wrap justify-center">
                    {Object.entries(cats).map(([key, label]: [string, any]) => (
                        <button
                            key={key}
                        ...
                        >
                            {label}
                            <span className={`ms-2 rtl:me-2 rtl:ms-0 px-2 py-0.5 rounded-full text-xs ${
            ...
            {lightboxIndex !== null && (
                ...
                >
                    {/* Image counter */}
                    <div className="absolute top-6 start-1/2 -translate-x-1/2 px-4 py-2 bg-brand-card-hover backdrop-blur-sm rounded-full text-white text-sm font-bold border border-white/20">
                    ...
                    {/* Thumbnail strip */}
                    <div className="absolute bottom-4 start-0 end-0 flex justify-center gap-2 px-4 overflow-x-auto scrollbar-hide">
                        {filtered.map((img, i) => (
                            <div
                                key={i}
                                onClick={e => { e.stopPropagation(); setLightboxIndex(i); }}
                                className={`w-14 h-14 shrink-0 rounded-sg overflow-hidden cursor-pointer transition-all duration-200 border-2 ${
                                    i === lightboxIndex
