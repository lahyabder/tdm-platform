...
export function Navbar({ locale, dict }: { locale: string; dict: any }) {
    ...
    const navLinks = [
        ...
        { name: locale === 'ar' ? 'الأخبار' : 'Actualités', path: `/${locale}/news`, icon: FileText },
                ...
                <div className="max-w-[1400px] mx-auto px-6 flex items-center justify-between gap-8">
                    ...
                    <div className="hidden xl:flex items-center justify-center gap-1">
                        {navLinks.slice(0, 6).map((link) => {
                            ...
                            return (
                                <Link key={link.path} href={link.path} className={`px-4 py-2 rounded-sm text-sm font-bold transition-all duration-200 ${isActive ? 'text-brand-green bg-brand-green/10' : 'text-slate-100 hover:text-brand-green hover:bg-white/5'}`}>
                                    {link.name}
                                </Link>
                            );
                        })}
                        ...
                        <div className="max-w-7xl w-full mx-auto px-6 grid md:grid-cols-2 lg:grid-cols-3 gap-12 relative z-10">
                                            ...
                                            return (
                                                <motion.div key={link.path} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + (index * 0.05) }}>
                                                    <Link
                                                        ...
                                                        className="group flex items-center gap-4 p-4 rounded-2xl bg-brand-card hover:bg-brand-card-hover border border-white/5 hover:border-white/20 transition-all backdrop-blur-sm"
                                                    >
                                                        <div className="w-12 h-12 rounded-xl bg-brand-card flex items-center justify-center border border-white/20 group-hover:scale-110 group-hover:bg-brand-green/20 group-hover:text-brand-green group-hover:border-brand-green/30 transition-all text-slate-300">
                            ...
                            >
                                <div className="space-y-6">
                                    ...
                                    <div className="pt-6 border-t border-white/20 mt-6 flex gap-2 w-full h-1">
